...
CREATE OR REPLACE VIEW mz_thread_detail AS 
 SELECT mbt.rootmessageid, mbt.threadid, ( SELECT mo.name
          ...
          WHERE mo.id = mua.organizationid AND mua.userid = mb.userid AND mb.messageid = mbt.rootmessageid) AS organizationname, ( SELECT mo.id
           FROM mz_organization mo, mz_user_addon mua, mbmessage mb
          WHERE mo.id = mua.organizationid AND mua.userid = mb.userid AND mb.messageid = mbt.rootmessageid) AS organizationid, mb.subject, ( SELECT count(*)
           FROM mbmessage mb, mz_mbmessage_addon mbaddon
          WHERE mb.threadid = mbt.threadid AND mb.messageid = mbaddon.message_id AND mbaddon.status = 'APPROVED_MESSAGE' AND mbaddon.active = true) AS quantity, mma.signature, mma.date_from, mma.date_to, mma.summary_id, mma.accepted, mb.userid, mb.username, 
        CASE
...
ALTER TABLE mz_thread_detail
  OWNER TO mamzdanie;
  
ALTER TABLE mz_mbmessage_addon
  OWNER TO mamzdanie;
